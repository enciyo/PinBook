...
sealed class AwesomeResult<out S> {
    ...
    data class UnknownError(
            ...
            val message: String = exception.localizedMessage ?: "") : AwesomeResult<Nothing>()
...
inline fun <S, T> Flow<AwesomeResult<List<S>>>.mapFlowList(crossinline transform: (S) -> T): Flow<AwesomeResult<List<T>>> = this.map { awesomeResult ->
    awesomeResult.map {
        it.map(transform)
    }
}